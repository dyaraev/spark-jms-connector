...
object ExampleCommands {
  ...
  private val receiverJobCommand = Command("receiver-job", "Spark job with a JMS source") {
    (CommonOpts.opts, ReceiverOpts.opts, MessageGenOpts.opts).mapN { (commonOpts, receiverOpts, genOpts) =>
      ...
      val result = for {
        ...
        _ <- ActiveMqBroker.withActiveMqBroker(amqAddress, amqDataPath) { () =>
          JmsReceiverJob.generatorFields.flatMap { fields =>
      ...
      val result = for {
        ...
        _ <- ActiveMqBroker.withActiveMqBroker(amqAddress, amqDataPath) { () =>
          MessageReceiver.withMessageReceiver(amqAddress, SenderOpts.QueueName, 1000.millis, logEmpty = false) { () =>
            FieldSpec.fieldGenerators(JmsSenderJob.FieldSpecs).flatMap { fields =>
              FieldSpec.toStructType(JmsSenderJob.FieldSpecs).flatMap { schema =>
                CsvFileGenerator.withGenerator(fields, inputPath, genOpts.interval, genOpts.numRecords) { () =>
  ...
  private object ReceiverOpts {
    ...
    private val receiveTimeoutOpt: Opts[Duration] = Opts
